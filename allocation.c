...
isl_stat virtual_allocation (isl_ctx * optionsHdl, pet_scop ** polyhedralModelPtr, unsigned numTasks) {
	...
	// Pointer to the remapped access relation 
	isl_union_map * remappedAccessRelationPtr = NULL;
	...
	for (int i = 0; i < numTasks; i++) {
...
#endif
		// May - reads remapping
		remappedAccessRelationPtr = isl_union_map_apply_range (pet_scop_get_may_reads(polyhedralModelPtr[i]), 
													isl_union_map_from_map(isl_map_copy(allocationRelationPtr)));
		
		if (remappedAccessRelationPtr == NULL)
			return isl_stat_error;
		...
		fflush(stdout);
		isl_union_map_dump(remappedAccessRelationPtr);
#endif
		// May - writes remapping
		remappedAccessRelationPtr = isl_union_map_apply_range (pet_scop_get_may_writes(polyhedralModelPtr[i]), 
													isl_union_map_from_map(isl_map_copy(allocationRelationPtr)));
		...
		remappedAccessRelationPtr = isl_union_map_apply_range (pet_scop_get_must_writes(polyhedralModelPtr[i]), 
													isl_union_map_from_map(isl_map_copy(allocationRelationPtr)));
